...
<ul>
  ...
  <li>
  Обязательно снабдите плату нормальным радиатором с простеньким вентилятором, продаются готовые комплекты для ORPI5. Иначе процессор бут тротлить, а это нам не надо, и так проблем хватает)
  </li>
</ul>
...
Больше примеров и моделей конвертированных под NPU rknn можно найти в репозитории:
...
Тут будет описание разработки и архитектура приложения. Само приложение будет отдельно в соответствующей директории.

<h4>Часть 7. "Обучение yollo5 на свой датасет"</h4>

Бывает так что готовая yollo отлично работает, да вот только нужные объекты не распознает. Например из-за необычного ракурса или из-за того что просто этих объектов нет в исходном датасете. Тогда есть возможность дообучить (не заного обучить) модель на нашем датасете. Это выгодно потому что дообучение происходит быстро, для обучения требестя меньше эпох обучения, рамер датасета в сотни раз меньше для полного обучения. По этим причинам обучение происходит быстрее. Да и есть полно сервисов онлайн которые позоляют прямо онлайн.

Мануал распишу позже. Сечас огрничксь двумя ссылками 
- дообучение [yollo5](https://www.youtube.com/watch?si=YiN3WfLR5jVPoCFY&v=GRtgLlwxpc4&feature=youtu.be)
- дообучение [yollo8](https://www.youtube.com/watch?v=fhzCwJkDONE)

При обучении yollo5 у меня обнаружилось что пути "поплыли" относительно виде мануала и я пофиксил это тем что вставлял полные пути к папкам в goole.colab, полный путь можно получить ели тыкнуть на три точки права от фала/папки. 

<h4>Часть 8. "Конвертация модели под rk3588"</h4>

мануал по конвертации будет основан на материале 
https://forum.khadas.com/t/convert-yolov8-model-to-rknn/20095/17
